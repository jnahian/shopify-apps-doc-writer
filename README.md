...
```
/shopify-apps-doc-writer:docs-setup            one-time wizard: auth → publish target → product context
/shopify-apps-doc-writer:write-docs <feature>  worktree → discover → shot manifest (gate 1) → capture
                                               → write → review draft (gate 2) → publish (gate 3, optional)
/shopify-apps-doc-writer:update-docs           worktree → detect copy/screenshot drift → re-shoot
                                               → re-publish (gate 3)
/shopify-apps-doc-writer:docs-deploy           build internal docs site → confirm → Cloudflare Pages URL
                                               → optional Slack heads-up (draft-only, never auto-sent)
/shopify-apps-doc-writer:docs-check            re-shoot every manifest → report which docs went stale
                                               (report-only: writes nothing, needs no gates)
/shopify-apps-doc-writer:docs-schedule         daily background sweep (launchd) → notice at your next session
```
...
`/shopify-apps-doc-writer:docs-schedule` (macOS) runs that same sweep daily in the background via
...
still happens through `/shopify-apps-doc-writer:docs-check`, with you present.

`/shopify-apps-doc-writer:write-docs` and `/shopify-apps-doc-writer:update-docs` open with a **preflight confirmation** that is not one of the three: before writing anything they ask for a base branch and a worktree name (default `docs/<feature-slug>`) and do all work in `.worktrees/<branch>`, so your checkout stays untouched. Decline it, or already be in a worktree, and they work in place.

`/shopify-apps-doc-writer:docs-deploy` has its own confirmation gate of the same kind, separate from the three above: it publishes a *projection* of `docs/` to a Cloudflare Pages URL (viewable by anyone with the link) and never touches `meta.json` — per-doc publish records keep their existing meaning.
...
Login, verification, and default capture all drive your installed **Google Chrome** — no separate browser download (`npx playwright install`) needed out of the box. You need Chrome installed and Node ≥ 20. Then in Claude Code, run `/shopify-apps-doc-writer:docs-setup`.

Other capture engines (`chromium`, `firefox`, `webkit`, `msedge`) can be selected per manifest, per config, or with `--browser`. The three downloadable engines auto-install on first use; `msedge` needs a vendor install like Chrome. Firefox and WebKit do work against a live admin (the saved Chrome session carries over), but their screenshots aren't byte-stable across repeat captures the way Chrome's are — **stick with `chrome` for any doc you'll re-check with `/shopify-apps-doc-writer:update-docs` or `/shopify-apps-doc-writer:docs-check`**, or you'll get drift reports for a UI that never changed.

> **Upgrading from before multi-engine capture?** If `~/.config/shopify-apps-doc-writer/<app>.json` has `capture.browser: "chromium"` (the old, inert default that `/shopify-apps-doc-writer:docs-setup` used to write), that value is now honored — it will trigger a one-time bundled-Chromium download and render with Chromium instead of Chrome. Delete the key to stay on Chrome.
...
commands/                            the six /shopify-apps-doc-writer:… commands listed above
...
scripts/update-check.js              drift detector for /shopify-apps-doc-writer:update-docs (--all: sweep for /shopify-apps-doc-writer:docs-check)
scripts/sweep.js                     unattended sweep runner for /shopify-apps-doc-writer:docs-schedule (launchd)
scripts/schedule-sweep.js            install/uninstall/status of the launchd job (macOS)
scripts/build-site.js                docs/ → static site for /shopify-apps-doc-writer:docs-deploy (Cloudflare Pages)
scripts/lib/                         config + Shopify admin helpers
```

## v2 backlog

Screenshot annotation (arrows/highlights/blur), `/shopify-apps-doc-writer:update-docs` re-publish diffing against a live external doc, demo-data seeding, multi-locale capture, docs-site publish targets. See the spec for details.
