...
const { parseArgs, resolveAppKey, loadConfig, sweepPath, CONFIG_DIR } = require('./lib/config');
...
/**
 * Refuse to schedule from a directory with no docs in it. The shim bakes in
 * the cwd install ran from and never revisits the choice, so a schedule
 * installed from the wrong place sweeps nothing and reports clean every night
 * — the one failure mode an unattended feature cannot let through.
 * @param {string} docsRepo
 * @param {string} outputDir
 */
function assertDocsRepo(docsRepo, outputDir) {
  const dir = path.resolve(docsRepo, outputDir);
  const hasDoc =
    fs.existsSync(dir) &&
    fs.readdirSync(dir).some((d) => fs.existsSync(path.join(dir, d, 'manifest.json')));
  if (!hasDoc) {
    throw new Error(
      `No docs found in ${dir} — run /shopify-apps-doc-writer:docs-schedule from your docs repo. ` +
        'A schedule installed elsewhere would sweep nothing and report clean forever.'
    );
  }
}
...
function shimContent({ appKey, docsRepo, nodePath }) {
  ...
  // The stale-pointer branch writes its own sweep record. Exiting with only a
  // log line leaves the last successful record in place, so the user hears
  // nothing for two days and then only the generic "looks stuck" notice —
  // never the sentence that says how to fix it.
  return `#!/bin/sh
# Generated by shopify-apps-doc-writer /shopify-apps-doc-writer:docs-schedule — do not edit.
...
if [ -z "$ROOT" ] || [ ! -d "$ROOT" ]; then
  ...
  printf '{"at":"%s","status":"error","message":"%s"}\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    "plugin-root pointer missing or stale — open a Claude Code session to refresh it, then the next scheduled sweep recovers on its own" \
    > ${shq(sweepPath(appKey))}
  exit 1
fi
...
function install({ appKey, at, docsRepo }) {
  ...
  assertDocsRepo(docsRepo, loadConfig(appKey).capture.outputDir);
...
function status(appKey) {
  ...
  if (fs.existsSync(sweepPath(appKey))) {
    try {
      ...
      // The reason, not just the status: for a stale plugin-root pointer this
      // sentence is the fix, and the log is the only other place it appears.
      if (record.message) console.log(`  ${String(record.message).split('\n')[0]}`);
    } catch {
...
function main() {
  if (process.platform !== 'darwin') {
    console.error('Scheduled sweeps are macOS-only in 0.4.0 (launchd). Linux cron is deferred.');
    process.exit(1);
  }
  ...
  try {
    const appKey = resolveAppKey(
      /** @type {string|undefined} */ (typeof args.app === 'string' ? args.app : undefined)
    );
...
module.exports = { parseAt, plistContent, shimContent, label, plistPath, shimPath, logPath, shq, xmlEscape, assertSafeAppKey, assertDocsRepo };
