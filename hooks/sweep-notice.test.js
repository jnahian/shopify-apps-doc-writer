...
assert.ok(drift !== null, 'drift record produces notice');
...
assert.ok(authExpired !== null && /docs-setup auth/.test(authExpired));
const challenged = notice({ at: '2026-08-13T03:00:00Z', status: 'bot-challenge' });
assert.ok(challenged !== null && /bot-challenged/.test(challenged) && /docs-check/.test(challenged) && !/manifest/.test(challenged), 'exit-30 contract: no manifest blame');
const errNotice = notice({ at: '2026-08-13T03:00:00Z', status: 'error', message: 'boom' });
assert.ok(errNotice !== null && errNotice.includes('/logs/storeseo.sweep.log'), 'error points at the log');
...
assert.ok(stuck !== null && /stuck/.test(stuck) && /--status/.test(stuck), `stale record → stuck notice: ${stuck}`);
