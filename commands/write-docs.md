...
4. **Capture is deterministic.** All screenshots come from `scripts/capture.js` executing the approved manifest — never from ad-hoc browsing.
5. **Read-only.** The manifest must never contain actions that mutate store data.
6. Canonical output is always `docs/<feature-slug>/` (index.md, manifest.json, meta.json, screenshots/). Publishing is an optional projection of it.
